...
class Like extends DbModel
{
    ...
    {
        $statement = (new Like)->prepare("SELECT COUNT(*) FROM post_like WHERE post_id = :post_id");
        $statement->bindValue(':post_id', $postId);
        $statement->execute();
    ...
    {
        $statement = (new Like)->prepare("SELECT COUNT(*) FROM post_like WHERE post_id = :post_id AND cus_id = :cus_id");
        $statement->bindValue(':post_id', $postId);
        $statement->bindValue(':cus_id', $customerId);
        $statement->execute();
